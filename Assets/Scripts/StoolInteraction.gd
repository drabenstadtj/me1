...
func sit_player():
	...
	player_body.input_enabled = false 

	# 👉 Start cutscene after sitting
	start_cutscene()
	
func start_cutscene():
	var cutscene_player = get_tree().get_root().get_node("World/SubViewportContainer/SubViewport/CutscenePlayer/CutscenePlayer")
